<script lang="ts">
	...
	let totalPages = $derived(PAGE_SIZE > 0 ? Math.ceil(totalElements / PAGE_SIZE) : 0);
	let isEmpty = $derived(processes.length === 0 && totalElements === 0 && !error);
	...
	<div class="container mx-auto px-6 lg:px-20">
		...
		{:else if isEmpty}
			...
			<div class="text-center py-12">
				<p class="text-muted-foreground text-sm">No hay procesos disponibles.</p>
			</div>
		{:else}
			<!-- Process Cards -->
						...
						<CardContent class="p-0">
							<div class="grid grid-cols-1 md:grid-cols-[1.25fr_1fr_1fr_1fr_1fr]">
								...
								<div class="p-5 flex flex-col gap-1 items-start">
									<h2 class="text-lg font-bold text-brand-red leading-tight">{process.name}</h2>
									<span class="text-xs text-brand-gray-400 uppercase tracking-wider font-medium">{process.scope}</span>
									{#if process.description !== null}
										<p class="text-sm text-brand-gray-800 mt-1">{process.description}</p>
									{/if}
								</div>

								<!-- Column 2: Compromiso -->
								<div class="p-4 m-2 rounded-lg border border-brand-gray-200/60 bg-brand-gray-50/30 flex flex-col gap-2 items-center text-center">
								...
								<div class="p-4 m-2 rounded-lg border border-brand-gray-200/60 bg-brand-gray-50/30 flex flex-col gap-2 items-center text-center">
									...
									<div class="space-y-2 w-full">
										<div>
											<span class="text-[11px] text-brand-gray-400 uppercase tracking-wider">Fecha</span>
											<p class="text-sm text-brand-black font-medium">{formatDateTime(process.results)}</p>
										</div>
									</div>
								</div>

								<!-- Column 5: Estado -->
								<div class="p-5 flex flex-col items-center justify-center gap-4 h-full">
									<Badge
										variant="outline"
										class={cn('text-xs font-semibold px-3 py-1', getStatusStyle(process.estatus))}
									>
